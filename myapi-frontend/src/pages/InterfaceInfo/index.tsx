...
const InterfaceInfo: React.FC = () => {
   ...
   * @param fields
   */
  const handleUpdate = async (updateValue: API.InterfaceInfoUpdateRequest) => {
   ...
   * @param fields
   */
  const handleAdd = async (fields: API.InterfaceInfo) => {    /*处理新建摁钮功能RuleListItem改成了InterfaceInfo*/
  ...
  return (
    <PageContainer>
      <ProTable<API.RuleListItem, API.PageParams>  /*ProTable */
        headerTitle={'查询表格'}
        actionRef={actionRef}
        rowKey="key"
        search={{
        ...
        //发起一个请求  加载，刷新，查询，怎么调用请求函数全交给组建托管，不用反复绑定  listInterfaceInfoByPageUsingGET分页获取接口信息
        request={async (
          params: {
            pageSize?: number;
            current?: number;
